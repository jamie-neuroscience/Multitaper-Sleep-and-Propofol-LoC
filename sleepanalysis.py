...
for subject in test_subjects:
    ...
    tmax = 30.0 - 1.0 / subject_raw.info["sfreq"]  # tmax in included
...
#this removes a line of noise which might come from interference
...
#this reads the timestamps of the file
...
#this loads the data
def load_case_edf(filepath, channel='Fpz-Cz', hz=100, return_drug=False):
    """
    Loads EEG data from the SleepEDF databse
    ...
    channel : str
        The EEG channel to extract


    Returns
    -------
    signal : np.ndarray
        1D EEG data from the specified channel.
    sfreq : float
        Sampling frequency.
    """
    import mne
    ...
    # Pick the desired EEG channel
    if channel not in raw.ch_names:
        raise ValueError(f"Channel '{channel}' not found in EDF file. Available channels: {raw.ch_names}")

    raw.pick_channels([channel])
    signal = raw.get_data()[0]  # Extract as 1D NumPy array
...
def bin_frequencies(mtsg, freqs, n_bins=100, fmin=0, fmax=50):
    # Define bin edges between fmin and fmax
    bins = np.linspace(fmin, fmax, n_bins + 1)
    binned_power = np.zeros((n_bins, mtsg.shape[1]))

    for i in range(n_bins):
        # Find indices of freqs within current bin
        idx = np.where((freqs >= bins[i]) & (freqs < bins[i + 1]))[0]
        if len(idx) > 0:
            # Average power across those frequencies, for all epochs
            binned_power[i, :] = mtsg[idx, :].mean(axis=0)
        else:
            # If no frequencies fall in bin (unlikely), set zero
            binned_power[i, :] = 0
    return binned_power